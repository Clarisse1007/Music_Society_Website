...
<body>
    ...
    <section>
        <div>
        <?php
        echo "<table> <tr> <td><h3>Booking ID</h3></td> <td><h3>Member ID</h3></td> <td><h3>Event ID</h3></td> <td><h3>Quantity</h3></td> <td><h3>Booking Date</h3></td> <td><h3>Status</h3></td> </tr>";
        ...
        if ($result_readRecord) {   
            while ($row = mysqli_fetch_array($result_readRecord, MYSQLI_ASSOC)){      
                echo "<tr align='left'><td><i>" . $row['booking_id'] . "</i></td><td>" . $row['member_id'] . "</td><td>" . $row['event_id']  . "</td><td>" . $row['quantity']  . "</td><td>" . $row['booking_date']  . "</td><td>" . $row['status']  . "</td></tr>"; 
            }  
        }
        echo "</table>";      
        ?>
